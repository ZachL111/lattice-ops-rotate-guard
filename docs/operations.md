...
`lattice-ops-rotate-guard` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for rotate scenarios with log and snapshot fixtures, replay consistency checks, and no credentials or hosted services.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 6.
